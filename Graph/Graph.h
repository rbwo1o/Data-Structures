...
class Graph
{
    ...
    public:
        ...
        */
        ...
        /* Function: hasCycle
        * Parameters: Vertex 
        * Return: Boolean representing a cycle
        * Searches the Graph at the given vertex location for a cycle
        */
        bool hasCycle(int vertex)
        {
            ...
            // vector of exploredVertices
            std::vector<int> exploredVertices;

            
            /* Initial Population of explored Vertices*/

            // for each Edge of initial Vertex
            for(int i = 0; i < this->Vertices->at(vertex)->edges.size(); i++)
            {
                // for each explored Vertex
                for(int j = 0; j < exploredVertices.size(); j++)
                {
                    // search for Edge in list of explored Vertices
                    if(this->Vertices->at(vertex)->edges.at(i)->name == exploredVertices.at(j))
                    {
                        return true;
                    }
                }
                // append Edge to explored Vertices
                exploredVertices.push_back(this->Vertices->at(vertex)->edges.at(i)->name);
            }


            /* Traversal Graph Population of explored Vertices */

            // for each explored Vertex
            for(int i = 0; i < exploredVertices.size(); i++)
            {
                // for each Edge of each explored Vertex
                for(int j = 0; j < this->Vertices->at(exploredVertices.at(i))->edges.size(); j++)
                {
                    // for each explroed Vertex
                    for(int k = 0; k < exploredVertices.size(); k++)
                    {
                        // search for Edge in list of explored Vertices
                        if(this->Vertices->at(exploredVertices.at(i))->edges.at(j)->name == exploredVertices.at(k))
                        {
                            return true;
                        }
                    }
                    // append Edge to explored Vertices
                    exploredVertices.push_back(this->Vertices->at(exploredVertices.at(i))->edges.at(j)->name);
                }
            }
